...
//		    	}
	...
	@RequestMapping(value = "/listLMSUsers", method = RequestMethod.GET)
	  public ModelAndView listLMSUsers(@RequestParam(name= "page", required = false) Integer pageNumber,HttpServletResponse response, HttpServletRequest request ) throws Exception {
		 User user = (User) request.getSession().getAttribute("user");
		 if(pageNumber == null) {
				pageNumber = 0;
			}
	...
	@RequestMapping(value = "/searchUsrs", method = RequestMethod.GET)
	 ...
	 @RequestMapping(value = "/lmsAdmins", method = RequestMethod.GET)
	  public ModelAndView lmsAdmins(@RequestParam(name= "page", required = false) Integer pageNumber, HttpServletResponse response, HttpServletRequest request, ModelMap modelMap ) throws Exception {
		 User user = (User) request.getSession().getAttribute("user");
		 if(pageNumber == null) {
				pageNumber = 0;
			}
	...
	@RequestMapping(value = "/addlmsadmin", method = RequestMethod.GET)
			...
			MultipartFile multipartFile = request.getFile("idusers");
				...
				for(User u : users) {
					...
					if(u1 == null){ // first time sent email
						...
						EmailGenericMessageThread client = new EmailGenericMessageThread(u.getEmail(), "Registration Credentials - "+propertyConfig.getBaseUrl(), html, propertyConfig);
	 ...
	 public @ResponseBody String shareModule( @RequestParam(required=false, name="meetingid") String meetingid, @RequestParam("moduleName") String moduleName, @RequestParam("classifier") String classifier, HttpServletRequest request, HttpServletResponse response) throws IOException {
	...
	classifier = URLDecoder.decode(classifier);
	 ...
	 @RequestMapping(value = "/fetchModuleData", method = RequestMethod.GET)
		@ResponseBody
		public String showModuleData(@RequestParam(name = "mname", required = true) String mname,HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		 User user = (User) request.getSession().getAttribute("user");
	 ...
	 @RequestMapping(value = "/fetchModuleDataForPreviewForAdmin", method = RequestMethod.GET)
		@ResponseBody
		public String showModuleDataForPreviewForAdmin(@RequestParam(name = "mname", required = true) String mname,HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) throws Exception {
		 User user = (User) request.getSession().getAttribute("user");
		 Module module = moduleService.findUniqueModule(mname, user.getCompanyId());
	 ...
	 @ResponseBody
	 public String fetchModulesForAdminUserByLicense(@RequestParam(name = "licname", required = true) String licname, HttpServletRequest request, HttpServletResponse response, ModelMap modelMap){
		 User user = (User) request.getSession().getAttribute("user");
	 ...
	 @ResponseBody 
	public String fetchModuleItemJ( @RequestParam(name = "moduleId", required = true) Long moduleId, HttpServletRequest request, HttpServletResponse response, ModelMap modelMap){
		User user = (User) request.getSession().getAttribute("user");
