 ...
 <div class="master-container">
                ...
                <div class="container-fluid">
                    ...
                    <nav class="">
                       ...
                       <a href="#" class="view-more active">
                            Cluster Management <i class="fa fa-angle-down"></i>
                        </a>
                        <div class="more-nav">
                            <a href="newSkills">
                                Skills
            ...
            <section class="content-section">
                <div class="container-fluid">
                    <div class="page-header mb-4">
                        <h1 class="my-auto">Cluster Management</h1>
                        <div class="quick-actions my-auto">
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalCenter" onclick="addCluster()">
                                Add New Cluster
                            </button>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 col-md-6 mb-3">
                                  <form action="newSearchClusters" method="get">
                                <div class="input-group mb-0">
                                    <input type="text" class="form-control" placeholder="Search Cluster" name="searchText" id="searchText" value="${param.searchText}">
                        		...
                        		<c:forEach  items="${clusters}" var="cluster" varStatus="loop">   
			                        <div class="col-xs-12 col-md-4 col-xl-3 mb-3">
			                            <div class="card">
			                                <div class="card-header">
			                                    <div class="actions">
			                                        ...
			                                        <div class="options" style="cursor: pointer;">
			                                            <a onclick="editCluster(${cluster.id})" class="text-warning">
			                                                <i class="fa fa-edit mr-2"></i> Edit
			                                            </a>
			                                            <a href="javascript:deleteCluster('${cluster.id}')" class="text-danger">
			                                ...
			                                <div class="card-body">
			                                    <h5 class="card-title">${cluster.clusterName}  </h5>
			                                    <p class="card-text">URL: <a href="${cluster.url} ">${cluster.url} </a></p>
			                                    <p class="card-text">Workspace: <span class="text-primary">${cluster.noOfWorkspaces}</span></p>
			                                    <p class="card-text">Capacity Reached: <span class="text-success">${cluster.capacityReached}</span></p>
			                                </div>
			                            </div>
			                        </div>
			                        </c:forEach>
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLongTitle">Create/Edit Cluster</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                              <form name="tenantForm"  method="post" modelAttribute="cluster" action="newSaveCluster">
				                        <div class="modal-body">
				                                <div class="row">
				                                    <div class="col-12">
				                                        <div class="form-group">
				                                            <label for="">Cluster Name</label>
				                                              <form:input path="cluster.clusterName" name="clusterName" id="clusterName" required="true" class="form-control"/>
				<!--                                             <input type="text" class="form-control"> -->
				                                        </div>
				                                    </div>
				                                    <div class="col-12">
				                                        <div class="form-group">
				                                            <label for="">Cluster URL</label>
				                                            <form:input path="cluster.url" name="url" id="url" required="true" class="form-control"/>
																	
																	<form:hidden path="cluster.id" id="clusterId"/>
				<!--                                             <input type="text" class="form-control"> -->
				                                        </div>
				                                    </div>
				                                    <div class="col-12">
				                                        <div class="form-group">
				                                            <label for="">Key Cloak Url</label>
				                                             <form:input path="cluster.keyCloakUrl" name="keyCloakUrl" id="keyCloakUrl" required="true" class="form-control"/>
				                                        </div>
				                                    </div>
				                                    <div class="col-12">
				                                        <div class="form-group">
				                                            <label for="">Cluster Capacity (NRT - Update with Caution)</label>
				                                             <form:input path="cluster.noOfWorkspaces" name="noOfWorkspaces" id="noOfWorkspaces" class="form-control"/>
				<!--                                             <input type="text" class="form-control"> -->
				                                        </div>
				                                    </div>
				                                </div>
        ...
        <script>
    		...
    		function deleteCluster(id){
			 	 Swal.fire({
				  title: 'Are you sure?',
				  text: "Do you want to delete this Cluster ?",
				  icon: 'warning',
				  ...
				  if (result.value) {
					 window.location = "newDeleteCluster?clusterId=" + id;
				  }
				})
	}
    		...
    		function addCluster(){
    				...
    				$("#keyCloakUrl").val("");
    				$("#noOfWorkspaces").val("");
    				$("#exampleModalLongTitle").text("Add Cluster")
    				$("#exampleModalCenter").modal("show");
        		}
    		
    		function editCluster(id){
            	$.get("editCluster?clusterId=" + id, function(data,	status) {
    				$("#clusterName").val(data.cluster.clusterName);
    				$("#url").val(data.cluster.url);
    				$("#clusterId").val(data.cluster.clusterId);
    				$("#keyCloakUrl").val(data.cluster.keyCloakUrl);
    				$("#noOfWorkspaces").val(data.cluster.noOfWorkspaces);
    				$("#exampleModalLongTitle").text("Edit Cluster")
    				$("#exampleModalCenter").modal("show");
             	});
        		}
    	      ...
    	      $('#search').on('click',function(){
          		...
          		if(text.length != 0){
          		window.location="newSearchClusters?searchText="+text;
          		}
          	    });
