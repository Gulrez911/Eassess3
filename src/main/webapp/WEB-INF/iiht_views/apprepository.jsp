    ...
    <body>
						...
						<tbody>
						 <c:forEach  items="${profiles}" var="profile" >  
						  <tr>
							...
							<td>	<a href='javascript:profile("${profile.candidateProfileUrl}")'  > Click</a> </td>
							<td>	<a href='javascript:comment("${profile.candidateProfileUrl}")'  > Click</a> </td>
						  </tr>
						 </c:forEach> 
						</tbody>
					  </table>
        ...
        <script>
        	...
        	function viewAppTrackerForJD(id) {
				window.location="showApplications?jid="+jid;
			}
			
			function viewSkillProfile(url){
				$(function() {
				    Swal.fire(
			       'Information',
			       'Skill Profile Not available as of yet.',
			       '${icon}'
			    )
			});
			}
			...
			function comment(url){
				...
				var url2="getComment"+url;
				  var w = 900;
    				var h = 800;
				 var left = (screen.width/2)-(w/2);
				var top = (screen.height/2)-(h/2);
				window.open (url2, 'User Profile', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width='+w+', height='+h+', top='+top+', left='+left);
